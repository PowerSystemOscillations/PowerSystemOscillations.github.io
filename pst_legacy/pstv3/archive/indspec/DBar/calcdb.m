function [rs,xs,Xm,rr,xr,dbf,nll,VoltAmp,tfl,isfl,flpf,fleff,tst,ist,eflag]=calcdb(vr,flc,fleff,flpf,flslip,stt,stc,nll,isat)
% syntax: [rs,xs,Xm,rr,xr,dbf,nll,VoltAmp,flpf,fleff,tfl,tst,ist,emat]=calcdb(vr,flc,fleff,flpf,flslip,stt,stc,nll,isat);
...
%           isfl    -   Full load current magnitude
%           flpf    -   Full load power factor
...
rs=[];xs=[];Xm=[];rr=[];xr=[];dbf = [];VoltAmp=[];tfl=[];isfl = [];tst=[];ist=[];eflag=0;
...
dbf = 2*rrs1/rr;
if rr<0
    uiwait(msgbox('rr negative','caldb error','modal'))
    return
end
...
x(1) = rr/flslip;x(2)=Xm;x(3)=xs;x(4)=rs;x(5) = dbf;
...
while (max(abs(y))>1e-12)&&n_c1<50
    n_c1=n_c1+1;
    [y,Jc] = jacmdb(x,fleff,flpf,stc,stt,flc,flslip,nll,isat);
    dx = Jc\y;
    x = x-dx;
    if (abs(max(x))>300)||(sum(isnan(x))~=0)||(sum(isinf(x))~=0)
        uiwait(msgbox('parameter values out-of-range','calcdb error','modal'))
        eflag = 1;
        return
    end
...
[zr,zi,xss,xrs,rre,xre,is1,ir1,eflag]=zdb(rs,xs,Xm,xr,rr,dbf,1,flslip,isat);
tfl = ir1*ir1*rre/flslip;
[zrs,zis,xss,xrs,rrse,xrse,iss,ir1s,eflag]=zdb(rs,xs,Xm,xr,rr,dbf,1,1,isat);
tst = ir1s*ir1s*rrse;
