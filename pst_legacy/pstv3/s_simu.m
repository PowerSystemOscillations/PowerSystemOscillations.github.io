...
if n_conv~=0
   ...
   if ndcr_ud~=0
      for j = 1:ndcr_ud
         sv = dcr_dc{j,1};
         if j==1
            xdcr_dc = zeros(size(sv.a,1),kdc);
            dxdcr_dc = zeros(size(sv.a,1),kdc);
         else
            xdcr_dc = [xdcr_dc;zeros(size(sv.a,1),kdc)];
            dxdcr_dc = [dxdcr_dc;zeros(size(sv.a,1),kdc)];
         end
      end
   ...
   if ndci_ud~=0
      for j = 1:ndci_ud
         sv = dci_dc{j,1};
         if j==1
            xdci_dc = zeros(size(sv.a,1),kdc);
            dxdci_dc = zeros(size(sv.a,1),kdc);
         else
            xdci_dc = [xsvc_dc;zeros(size(sv.a,1),kdc)];
            dxdci_dc = [dxsvc_dc;zeros(size(sv.a,1),kdc)];
         end
      end
...
if n_svc~=0
   ...
   if n_svcud~=0
      d_sig = zeros(n_svcud,k);
      for j = 1:n_svcud
         sv = svc_dc{j,1};
         if j==1
            xsvc_dc = zeros(size(sv.a,1),k);
            dxsvc_dc = zeros(size(sv.a,1),k);
         else
            xsvc_dc = [xsvc_dc;zeros(size(sv.a,1),k)];
            dxsvc_dc = [dxsvc_dc;zeros(size(sv.a,1),k)];
         end
      end
   else
      xsvc_dc = zeros(1,k);
...
if n_tcsc~=0
   ...
   if n_tcscud~=0
      ...
      for j = 1:n_tcscud
         ...
         if j==1
            xtcsc_dc = zeros(size(sv.a,1),k); % tcsc damping control states
            dxtcsc_dc = zeros(size(sv.a,1),k);% tcsc dc rates of chage of states
         else
            xtcsc_dc = [xtcsc_dc;zeros(size(sv.a,1),k)];% in order of damping controls
            dxtcsc_dc = [dxtcsc_dc;zeros(size(sv.a,1),k)];
         end
      end
   else
      xtcsc_dc = zeros(1,k);
