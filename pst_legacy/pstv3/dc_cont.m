...
global  r_idx  i_idx n_dcl  n_conv  rec_ac_bus  inv_ac_bus
global  Vdc  i_dc  alpha  gamma Vdc_ref
   ...
   if flag == 0
      ...
      if i == 0 
         ...
         if ~isempty(vrmax)
            svrm=int2str(vrmax);
            disp(svrm)
            error('v_conr greater than maximum limit at some rectifiers')
         end
         if ~isempty(vrmin)
            svrm=int2str(vrmin);
            disp(svrm)
            error('v_conr less than minimum limit at some rectifiers')
         end
         ...
         if ~isempty(vimax)
            svim=int2str(vimax);
            disp(svim)
            error('v_coni greater than maximum')
         end
         if ~isempty(vimin)
            svim=int2str(vimin);
            disp(svim)
            error('v_coni less than minimum')
         end
