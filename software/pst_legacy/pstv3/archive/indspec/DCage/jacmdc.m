function [y,Jc] = jacmdc(x,fleff,flpf,stc,stt,flc,flslip,nll,dr,isat);
y = zeros(5,1);J = zeros(5);
angfl = angle(flc);
rr1 = x(1)*flslip;Xm = x(2);xs = x(3);xr = x(3);rs = x(4);rr2 = x(5)*flslip;xr2 = (rr1+rr2)*dr;
[zr,zi,xss,xrs,rre,xre,is1,ir1,eflag]=zdc(rs,xs,Xm,xr,xr2,rr1,rr2,1,flslip,isat);
tfl = ir1*ir1*rre/flslip;
is = 1/(zr+i*zi);
[zrs,zis,xss,xrs,rrse,xrse,is1s,ir1s,eflag]=zdc(rs,xs,Xm,xr,xr2,rr1,rr2,1,1,isat);
tst = ir1s*ir1s*rrse;
...
y(5) = tst/tfl/stt -1;
y1 = y;Jc = J;
rr1 = (x(1)+1e-6)*flslip;xr2 = (rr1+rr2)*dr;
[zr,zi,xss,xrs,rre,xre,is1,ir1,eflag]=zdc(rs,xs,Xm,xr,xr2,rr1,rr2,1,flslip,isat);
tfl = ir1*ir1*rre/flslip;
is = 1/(zr+i*zi);
[zrs,zis,xss,xrs,rrse,xrse,is1s,ir1s,eflag]=zdc(rs,xs,Xm,xr,xr2,rr1,rr2,1,1,isat);
...
Jc(:,4) = 1e6*(y4-y);
y5 = y;
rr1 = x(1)*flslip;Xm = x(2);xs = x(3);xr = x(3);rs = x(4);rr2 = x(5)*flslip;
rr2 = (x(5)+1e-6)*flslip;xr2 = (rr1+rr2)*dr;
[zr,zi,xss,xrs,rre,xre,is1,ir1,eflag]=zdc(rs,xs,Xm,xr,xr2,rr1,rr2,1,flslip,isat);
tfl = ir1*ir1*rre/flslip;
is = 1/(zr+i*zi);
[zrs,zis,xss,xrs,rrse,xrse,is1s,ir1s,eflag]=zdc(rs,xs,Xm,xr,xr2,rr1,rr2,1,1,isat);
