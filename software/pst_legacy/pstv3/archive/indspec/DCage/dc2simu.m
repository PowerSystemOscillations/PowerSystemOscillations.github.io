function [xff,mcurr,xslow,te,tes,requ,xequ,tff,ts]= dc2simu(V,f,tfr,tld,rs,xs,Xm,rr,rr2,xr,xr2,isat,H,tsim);
% syntax: [xff,mcurr,xslow,te,tes,requ,xequ,tff,ts]= dc2simu(V,f,tfr,tld,rs,xs,Xm,rr,rr2,xr,xr2,isat,H,tsim);
% simulates a double cage induction motor starting from a constant voltage
% source
% Inputs
...
% rr2   -   second cage rotor reactance PU
% xr    -   rotor leakage reactance PU
% xr2   -   inter-cage rotor reactance PU
...
[zr,zi,xss,xrs,rre,xre,is1,ir1,eflag]=zdc(rs,xs,Xm,xr,xr2,rr,rr2,V,1,isat);
Xr = xre+Xm;Xs = xss+Xm;
Xsp = xss+xre*Xm/(xre+Xm);
Xrp = xre+xss*Xm/(xss+Xm);
requ = rre;xequ = xre;
mcurr = [];
while ns<length(ts)-1
    ...
    sm = stsp(asmat,bs,eye(4),zeros(4,1));% forms a state space object for the stator equations
    [xfast,tf,xfend] = response(sm,1,tf,xfend);% solves for the stator transients assuming slip constant
    ...
    [zr,zi,xss,xrs,rre,xre,is1,ir1,eflag]=zdc(rs,xs,Xm,xr,xr2,rr,rr2,V,abs(xslow(ns+1)),isat);
    Xr = xre+Xm;Xs = xss+Xm;
    Xsp = xss + xre*Xm/(xre+Xm);
    Xrp = xre + xss*Xm/(xss+Xm);
