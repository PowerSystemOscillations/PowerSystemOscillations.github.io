% Three Area Test Case
% subtransient generator models
% dc exciters
% turbine/governor
% 50% constant current/50% constant impedance loads
% active and reactive load modulation at load buses (4 and 14)
% reactive load modulation at buses 3,13 and 101
% HVDC link between non-synchronous systems
...
bus = [...
   ...
   102 1.03      -6.  1.5    0.5   0.00  0.00  0.00  0.00 1  99.0  -99.0  22.0   1.1  .9;
   103 1.0       0.0  0.0    0.0   0.00  0.00  0.00  3.00 3  0.0     0.0  230.0  1.5  .5;
   104 1.0       0.0  0.0    0.0   0.00  0.00  0.00  0.00 3  0.0     0.0  110.0  1.2  .5;
   105 1.0       0.0  0.0    0.0   0.00  0.00  0.00  0.00 3  0.0     0.0  110.0  1.2  .5;
   106 1.0       0.0  0.0    0.0   0.00  0.00  0.00  0.00 3  0.0     0.0  230.0  1.5  .5];
...
13  103 0.0     0.01    0.00    1.0  0. 5.0 0.5 0.005;     
13  101 0.011   0.11     0.1925  1.0  0. 0.  0.  0.;
13  101 0.011   0.11     0.1925  1.0  0. 0.  0.  0.;
13  120 0.001   0.01     0.0175  1.0  0. 0.  0.  0.;
110 120 0.0025  0.025    0.0437  1.0  0. 0.  0.  0.;
102 103 0.0     0.033    0.00     1.0  0. 0.  0.  0.;
103 104 0.0     0.05    0.00    1.0  0. 5.0 0.5 0.005;
106 105 0.0     0.05    0.00    1.0  0. 5.0 0.5 0.005;
106 3  0.011   0.110   0.1925  1.0  0. 0.  0.  0.];

disp('HVDC line between buses 105 and 106')
...
dcsp_con = [...
1  104  1  500  3    4   5   30;
2  105  2  500  3    4  18   25];
...
4 12 900 0.200  0.0025  1.8  0.30  0.25 8.00  0.03...
                       1.7  0.55  0.25 0.4   0.05...
                       6.5  0  0  12;
5 102 300 0.200  0.0025  1.8  0.30  0.25 8.00  0.03...
                       1.7  0.55  0.25 0.4   0.05...
  3.5  0  0  102];
           

% static exciter on gens 2,5 ; dc exciters on gens 1,3 and 4;  pss on gens 5 
...
pss_con = [...
     ...
     1  5  100  10 0.07 0.02 0.07 0.02 0.2 -0.05;
     ];
...
%NOTE: any dc buses must be last in the load_con matrix
load_con = [...      
4   0  0   .5  0;
14  0  0   .5  0;
104 0  0   0   0;% dc rectifier bus
105 0  0   0   0];% dc inverter bus
disp('0.5 constant current load')
disp('active load modulation at buses 4 and 14, reactive load modulation at buses 3,4,13,14')
...
1 4  100 1 -1  1   0.05;
2 14  100 1  -1  1  0.05];
svc_dc = [];tcsc_dc=[];dci_dc=[];dcr_dc=[];
cdnow = cd;
cdps('chapter10\results');
load contdc
cd(cdnow);
dci_dc=[];
dcr_dc{1}=dif_stsp(0.01).*wo_stsp(.1).*sdc2;dcr_dc{2}=1;dcr_dc{3}=3;dcr_dc{4}=13;
dcr_dc{5}=1;dcr_dc{6}=-1;dcr_dc{7}=11;
...
0.2   13  101  1.0    1.0       3    0.001; %line to line fault 
0.25   0    0    0      0       0    0.001; %clear fault at bus 3
0.30   0    0    0      0       0    0.001;%clear remote end
